(function () {
  ...
  function pmCard(pm) { return shell(pm, 'pm-card', `<span class="pm-card-label">PM Procedure</span><h3>${escapeHtml(pm.title)}</h3><p>${escapeHtml(pm.description)}</p><div class="badges"><span class="badge asset">${escapeHtml(pm.assetType)}</span><span class="badge manufacturer">${escapeHtml(pm.manufacturer)}</span><span class="badge model">${escapeHtml(pm.model)}</span></div><p class="date"><em>Last Revision: ${escapeHtml(pm.dateAdded)}</em></p>`); }
